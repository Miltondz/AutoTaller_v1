      ...
      <section className="relative py-20 sm:py-28 px-4 sm:px-6 lg:px-8 bg-slate-800">
        ...
        <div className="relative max-w-4xl mx-auto text-center text-white">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2, ease: 'easeOut' }} className="text-lg sm:text-xl md:text-2xl text-slate-200 max-w-3xl mx-auto">
            Conoce la trayectoria y la filosofía que me convierten en tu mejor aliada para el aprendizaje musical en Venezuela.
          </motion.p>
        </div>
      </section>
      ...
      <Section className="py-16 sm:py-24 bg-slate-50 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-800 mb-4">Mi Trayectoria Profesional</h2>
            <p className="text-lg sm:text-xl text-slate-600 max-w-3xl mx-auto">Un resumen de mi experiencia y formación, dedicada a la excelencia en la educación musical en ciudades como Maracaibo, Caracas y Mérida.</p>
          </div>
          <div className="relative">
            <TimelineItem icon={<GraduationCap />} title="Educación Formal">Licenciatura en Educación, Mención Música (UNEFM). Estudios avanzados de Cuatro, Guitarra y Piano en prestigiosas escuelas de música de Venezuela.</TimelineItem>
            <TimelineItem icon={<Briefcase />} title="Experiencia Docente">He tenido el honor de ser docente en el Conservatorio Musical del Estado Falcón (COMESFAL) y en la Universidad Nacional Experimental Francisco de Miranda (UNEFM).</TimelineItem>
            <TimelineItem icon={<Users />} title="Dirección y Liderazgo">Fundadora de la agrupación infantil "Cantores del 23" y Directora Musical de la Coral UNEFM, fomentando el talento joven.</TimelineItem>
            <TimelineItem icon={<Music />} title="Agrupaciones Corales">Miembro activo y coralista en agrupaciones de renombre como la Coral del Centro de Refinación Paraguaná y la Coral Cotraedup.</TimelineItem>
            <TimelineItem icon={<BarChart />} title="Desarrollo Profesional">Locutora Profesional Certificada y participante activa en talleres de pedagogía musical para mantenerme a la vanguardia de la enseñanza.</TimelineItem>
            <TimelineItem icon={<Heart />} title="Impacto Comunitario" isLast>Colaboradora del Proyecto de Acción Social por la Música Simón Bolívar y fundadora de proyectos para niños de bajos recursos.</TimelineItem>
          </div>
        </div>
      </Section>
      ...
      <section className="py-24 bg-amber-600 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center text-white">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8, ease: 'easeOut' }}>
            <h2 className="text-3xl sm:text-4xl font-bold mb-4">¿Listo para Empezar tu Viaje Musical?</h2>
            <p className="text-lg sm:text-xl mb-10 text-amber-100 max-w-2xl mx-auto">Ya sea que estés en Punto Fijo, Falcón o cualquier parte de Venezuela, estoy aquí para ayudarte. ¡Reserva tu clase personalizada hoy!</p>
